...
스토리북처럼 dist에 파일을 빌드를 해도 해결이 되지 않았고 차이점을 확인하기 위해 allowJs라는 속성을 변경한 후에 에러가 해결 됐습니다.

하지만 다시 속성을 원래대로 했더니 에러가 해결된 상태였고, 이전에 편집기의 tsc 캐시 삭제 및 재실행을 반복했었던 상황이기에 정확한 문제는 찾지 못했고 에러는 해결했습니다.

위 문제들을 해결하면서 이런 이슈가 발생하지 않도록 패키지를 dist로 확실히 빌드한 뒤 배포하고, 설정 충돌이 생기지 않도록 주의하는 것이 중요하다고 느꼈습니다.
