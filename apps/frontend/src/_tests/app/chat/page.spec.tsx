...
describe('인터뷰 페이지 통합 테스트', () => {
  ...
  describe('인터뷰 시작 및 초기 상태', () => {
    it('인터뷰 시작시 초기 인사말이 표시된다', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('안녕하세요. 간단히 자기소개 부탁드립니다.')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('사용자 응답 프로세스', () => {
    it('녹음 후 STT 변환 및 AI 응답을 정상적으로 처리한다', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('안녕하세요. 간단히 자기소개 부탁드립니다.')).toBeInTheDocument();
        expect(screen.queryByText('안녕하세요, 저는 개발자입니다.')).not.toBeInTheDocument();
      });
      ...
      await waitFor(() => {
        expect(screen.getByText('안녕하세요, 저는 개발자입니다.')).toBeInTheDocument();
      });
    });
