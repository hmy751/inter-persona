...
const validatePasswordConfirm = (passwordConfirm: string, originalPassword: string) => {
  if (!passwordConfirm) return '비밀번호를 입력해주세요.';
  if (passwordConfirm.length < 10) return '비밀번호는 최소 10자 이상이어야 합니다.';

  const hasUpperCase = /[A-Z]/.test(passwordConfirm);
  const hasLowerCase = /[a-z]/.test(passwordConfirm) && /[a-z].*[a-z]/.test(passwordConfirm); // 소문자 최소 2개
  ...
  if (originalPassword !== passwordConfirm) return '비밀번호가 일치하지 않습니다.';
...
const validateProfileImage = (file: File | null) => {
  ...
  if (!acceptedTypes.includes(file.type)) return 'JPG, PNG 형식의 이미지만 업로드 가능합니다.';
...
export default function SignupForm() {
  ...
  const [profileImageError, setProfileImageError] = useState<string | undefined>();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log(profileImage, emailField.value, nameField.value, passwordField.value, passwordConfirmField.value);
  };

  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <Field label="프로필 이미지" elementHeight="var(--space-12)" message={profileImageError}>
        <ProfileInput size="xl" setImage={file => setProfileImage(file)} onValidate={validateProfileImage} />
      </Field>
